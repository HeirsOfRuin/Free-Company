...
def available_offers(company, rng):
    ...
    for f in FACTIONS:
        ...
        pay_per_lance = base_rate * (1 + rep / 200.0) * (1 + company.dread / 300.0)
        pay_per_lance = max(pay_per_lance, 1.0)
        risk = rng.uniform(0.03, 0.09)
...
def resolve_contract_season(company, faction, pay_per_lance, risk, rng):
    ...
    if roll < win_chance:
        ...
        casualties = risk * company.lances * rng.uniform(0.3, 0.8)
        company.reputation[faction] = min(100, company.reputation[faction] + rng.uniform(5, 12))
        ...
        plunder = 0
        casualties = risk * company.lances * rng.uniform(1.0, 2.2)
...
def resolve_raid_season(company, target, rng):
    ...
    if rng.random() < 0.15:
        casualties = rng.uniform(0.02, 0.06) * company.lances
        company.lances = max(0, company.lances - casualties)
...
def update_loyalty(company, paid, heavy_loss, got_plunder):
    if paid and got_plunder:
        ...
        company.loyalty = max(0, company.loyalty - 15)
...
def run_strategy(strategy_name, rng, verbose=False):
    c = Company()
    ...
    for season in range(TOTAL_SEASONS):
        ...
        if action == "contract" and offer:
            ...
            c.under_contract_with = None
            c.loyalty = min(100, c.loyalty + 8)
            c.dread = max(0, c.dread - 3)
            # small garrison income
            income = rng.uniform(20, 80)
            c.treasury += income
...
def main():
    ...
    for strat in strategies:
        rng = random.Random(1234)  # same seed stream per strategy for comparability
        runs = [run_strategy(strat, random.Random(seed)) for seed in range(n_runs)]
